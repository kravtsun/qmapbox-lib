...
public:
    ...
    typedef std::vector<GeometryCoordinates>::vector vector;
//    std::vector<>::vector(::std::initializer_list<value_type> _Ilist)

    GeometryCollection() = default;

    GeometryCollection(const GeometryCoordinates &gc)
        : vector(1, gc)
    {}

//    GeometryCollection(size_type _Count, const value_type &_Val)
//        : vector(_Count, _Val)
//    {}

//    GeometryCollection(::std::initializer_list<value_type> _Ilist, const _Alloc& _Al = vector::allocator_type())
//        : vector(_Ilist, _Al)
//    {}
};
...
//        std::vector<mapbox::geometry::point<int16_t>> v;
//        v.push_back(geom);
        GeometryCoordinates gcs({geom});
        return {gcs};
